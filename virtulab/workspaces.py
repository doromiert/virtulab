...
class WorkspaceStore:
    ...
    def _migrate(self) -> None:
        with self._connection() as connection:
            ...
            for rack_row in connection.execute(
                "SELECT id, workspace_id FROM devices WHERE profile_id = 'server-rack'"
            ).fetchall():
                self._restack_rack(connection, rack_row["workspace_id"], rack_row["id"])
    ...
    def mount_device(
        ...
        with self._connection() as connection:
            ...
            hardware = json.loads(row["hardware_json"])
            previous_rack_id = hardware.get("rackId")
            if rack_id is None:
            ...
            for affected_rack in {previous_rack_id, rack_id} - {None}:
                self._restack_rack(connection, workspace_id, str(affected_rack))
            self._touch(connection, workspace_id)
        return self.snapshot(workspace_id)

    def _restack_rack(self, connection: sqlite3.Connection, workspace_id: str, rack_id: str) -> None:
        mounted = []
        for row in connection.execute(
            "SELECT id, hardware_json FROM devices WHERE workspace_id = ? AND id != ?",
            (workspace_id, rack_id),
        ).fetchall():
            hardware = json.loads(row["hardware_json"])
            if hardware.get("rackId") == rack_id:
                mounted.append((row["id"], hardware))
        mounted.sort(key=lambda item: (int(item[1].get("rackUnit", 48)), item[0]))
        next_unit = 1
        for mounted_id, hardware in mounted:
            hardware["rackUnit"] = next_unit
            next_unit += int(hardware.get("rackSpan", 10))
            connection.execute(
                "UPDATE devices SET hardware_json = ? WHERE id = ?",
                (json.dumps(hardware), mounted_id),
            )
        ...
        required_units = max(1, next_unit - 1)
        rack_hardware["rackUnits"] = max(int(rack_hardware.get("rackUnits", 1)), required_units)
        connection.execute(
            "UPDATE devices SET hardware_json = ? WHERE id = ?",
